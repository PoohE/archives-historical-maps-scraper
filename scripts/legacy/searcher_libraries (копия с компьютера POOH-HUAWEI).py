...
UA = "Mozilla/5.0 (research bot; contact: down.pooh@gmail.com)"
...
GPIB_ELIB   = "http://elib.shpl.ru/ru/nodes"
...
def _search_gpib(query: str, year_from: int | None, year_to: int | None,
                 max_pages: int) -> Iterator[LibraryRecord]:
    """
    ГПИБ: ищет через электронный каталог ИРБИС (unis.shpl.ru) и
    цифровую библиотеку (elib.shpl.ru/ru/nodes?search=<query>).
    """
    # 1. Электронная библиотека elib.shpl.ru
    elib_ok = _check(GPIB_ELIB)
    if elib_ok:
        for page in range(1, max_pages + 1):
            ...
            if page > 1:
                params["page"] = page
            try:
                resp = _get(GPIB_ELIB, params=params)
            except Exception as e:
                print(f"[ГПИБ] Ошибка elib стр.{page}: {e}")
                break

            soup = BeautifulSoup(resp.text, "lxml")
            items = soup.select(
                "li.search-result, .search-results li, "
                ".node-search-result, article.node"
            )
            if not items:
                break
        ...
        print("[ГПИБ] elib.shpl.ru недоступен.")
...
def _search_etomesto(query: str, year_from: int | None, year_to: int | None,
    ...
    MAP_KW = ("карт", "план", "схем", "атлас", "съёмк", "чертёж",
              "топограф", "межеван", "трёхвёрст", "верстн")
    ...
    for region in regions_to_check:
        ...
        for link in links:
            ...
            if not title or len(title) < 5:
                continue
            if not any(kw in title.lower() for kw in MAP_KW):
...
def _search_retromap(query: str, year_from: int | None, year_to: int | None,
    ...
    try:
        ...
        soup = BeautifulSoup(resp.text, "lxml")
        MAP_KW = ("карт", "план", "атлас", "схем", "съёмк", "топограф")
        ...
        for link in soup.select("a[href]"):
            ...
            if not (has_region or has_kw_from_query):
                continue
            if not any(kw in title_low for kw in MAP_KW):
                continue
...
def _search_southklad(query: str, year_from: int | None, year_to: int | None,
    ...
    MAP_KW = ("карт", "план", "схем", "атлас", "чертёж", "съёмк",
              "топограф", "межеван", "уезд", "губерн")
