...
_shared = Path(__file__).resolve().parents[2] / "Каталогизация"
sys.path.insert(0, str(_shared))

# Плоские импорты локальных адаптеров (triggers, territories и др.).
# Общая папка _shared остаётся первой, чтобы `from modules import ...`
# разрешался в общий каталогизационный слой.
_local_modules = Path(__file__).resolve().parent / "modules"
sys.path.insert(1, str(_local_modules))
