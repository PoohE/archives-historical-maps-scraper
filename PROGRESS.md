...
Расширить подтверждённый адаптер РГИА на четыре губернии и отдельно проверить
режим «Дела»; после этого перейти к frameset-поиску ГАРФ и разрешённому
...
- РГИА: адаптер исправлен; пробный запрос дал 9 карточек фондов с названиями,
  номерами фондов и датами через JSON endpoint `/ajax/obj/...`.
