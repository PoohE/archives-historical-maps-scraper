...
Доработать и runtime-проверить следующий незавершённый федеральный адаптер —
`searcher_rgia.py`; после этого перейти к frameset-поиску ГАРФ и разрешённому
...
- РГИА и ГАРФ: частично проверены; карточки/дела не извлечены полностью.
