...
Отдельно проверить режим «Дела» в РГИА и свести четыре территориальных прогона
в CSV; после этого перейти к frameset-поиску ГАРФ и разрешённому
браузерному маршруту для НЭБ/Росархива. Не запускать массовые прогоны НЭБ через
`requests` до устранения блокировки 403.
...
- ГАРФ: частично проверен; дела не извлечены полностью.
...
- Frameset ГАРФ требует URL контент-фрейма из DevTools (Doc/All), а не URL
  внешней оболочки.
