...
def get_session(s: requests.Session, debug: bool = False) -> str:
    """
    Получает session id (SES) от OPAC-Global.
    Пробует несколько подходов: прямой GET, запрос страницы поиска.
    Возвращает SES-строку или '' если не удалось.
    """
    # Вариант A: GET /cgiopac/opacg/opac.exe без параметров → из куки или мета-тега
    try:
        r = s.get(SEARCH_URL, timeout=15)
        # Ищем SES в ответе
        m = re.search(r"SES=([A-Za-z0-9+/=]+)", r.text)
        if m:
            ses = m.group(1)
            if debug:
                print(f"[СО] SES получен (вариант A): {ses[:20]}...")
            return ses
    except Exception as e:
        if debug:
            print(f"[СО] Вариант A ошибка: {e}")

    # Вариант B: запрос с ACT=LOGIN — для публичного доступа (без пароля)
    try:
        r = s.get(SEARCH_URL, params={"ACT": "LOGIN", "LNG": "RUS"}, timeout=15)
        m = re.search(r"SES=([A-Za-z0-9+/=]+)", r.text)
        if m:
            ses = m.group(1)
            if debug:
                print(f"[СО] SES получен (вариант B): {ses[:20]}...")
            return ses
        if debug:
            print(f"[СО] Вариант B: SES не найден. HTML: {r.text[:300]}")
    except Exception as e:
        if debug:
            print(f"[СО] Вариант B ошибка: {e}")

    # Вариант C: из cookies
    if "SES" in s.cookies:
        ses = s.cookies["SES"]
        if debug:
            print(f"[СО] SES из куки: {ses[:20]}...")
        return ses

    if debug:
        print("[СО] SES не получен — попробуем поиск без SES (возможно публичный доступ)")
    return ""
